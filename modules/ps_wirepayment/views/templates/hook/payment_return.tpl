...
{if $status == 'ok'}
    ...
    {if $showPaymentNextStep == 'show'}
        <p>
            Уважаемый @firstname @lastname, спасибо за ваш заказ №@id - @reference. С Вами в ближайшее время свяжется наш менеджер для подтверждения заказа. Если вам кажется, что мы потеряли Ваш заказ, позвоните нам по телефону 88001234567 или напишите на почту zakaz@test. Обязательно укажите в теме письма о каком заказе идет речь, чтобы мы могли быстрее Вам помочь!
        </p>
    {/if}
{else}
    <p class="warning">
